...
def single_snp_test_tractor(phgeno, anc, theta, odds_ratio, anc_effect, n_sim=50, seed=1234, case_prevalence=0.1, control_ratio=2.5):
    """
    ...
    sim_list = admix.simulate_phenotype_cc_1snp_tractor(phgeno=phgeno, 
                                                        anc=anc,
    ...
    for snp_i, snp_sim in enumerate(sim_list):
        ...
        for sim_i in range(n_sim):
            ...
            print(f"Number of cases: {np.sum(mix_pheno[study_index] == 1)}, number of controls: {np.sum(mix_pheno[study_index] == 0)}")
